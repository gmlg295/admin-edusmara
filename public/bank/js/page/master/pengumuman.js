...
const ApiData =  new MyFetch('/pengumuman/getData', { method: 'GET'});
// Seed demo
...
async function saveAnnouncement(status){
    ...
    const submitData =  new MyFetch('/pengumuman/add', 
            {
                method: 'POST',
    ...
    if(editId !== '' && editId !== null && editId !== undefined){
        ...
        if (files) {
            formData.append('isUpdateFile', 'true');
             ...
             const updateData =  new MyFetch('/pengumuman/update/' + editId, 
                                {
                                    method: 'POST',
                                    csrfHash: document.getElementById('csrf_edusmara').textContent,
                                    params : formData
